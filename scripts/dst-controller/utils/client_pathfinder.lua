...
local TrailblazerIntegration = require("dst-controller/integrations/trailblazer")
...
local CONFIG = {
    ...
    MOVE_INTERVAL = 0.05,        -- 移动指令间隔（秒）- 需要足够频繁才能保持行走
    TRAILBLAZE_REISSUE_INTERVAL = 0.35, -- Trailblazer 指令重发间隔（秒）
...
local pathfinding_state = {
    ...
    last_trailblaze_waypoint = 0,
    trailblaze_in_flight = false,
}
...
local function SubmitTrailblazeAction(player, controller, waypoint, waypoint_index)
    local destination = G.Vector3(waypoint.x, 0, waypoint.z)
    local locomotor = player.components and player.components.locomotor

    if locomotor and locomotor.Trailblaze then
        local bufferedaction = G.BufferedAction(player, nil, G.ACTIONS.TRAILBLAZE, nil, destination)
        local ok = pcall(function()
            locomotor:Trailblaze(destination, bufferedaction, false)
        end)
        if ok then
            print(string.format(
                "[ClientPathfinder] Trailblazer locomotor started for waypoint %d: (%.1f, %.1f)",
                waypoint_index,
                waypoint.x,
                waypoint.z
            ))
            pathfinding_state.last_trailblaze_waypoint = waypoint_index
            pathfinding_state.trailblaze_in_flight = true
            return
        end

        print("[ClientPathfinder] Trailblazer locomotor call failed, falling back to DoAction")
    end

    local action = G.BufferedAction(player, nil, G.ACTIONS.TRAILBLAZE, nil, destination)
    controller:DoAction(action)
    print(string.format(
        "[ClientPathfinder] Trailblazer action submitted for waypoint %d: (%.1f, %.1f)",
        waypoint_index,
        waypoint.x,
        waypoint.z
    ))
...
local function MoveToNextWaypoint()
    ...
    -- 优先使用 Trailblazer 执行路径点移动（保留自研路径规划和可视化）
    if pathfinding_state.use_trailblazer then
        if not pathfinding_state.trailblaze_in_flight or
           pathfinding_state.last_trailblaze_waypoint ~= pathfinding_state.current_waypoint then
            print(string.format("[ClientPathfinder] Trailblazer target waypoint %d: (%.1f, %.1f), dist: %.1f, dir: (%.2f, %.2f)",
                pathfinding_state.current_waypoint, waypoint.x, waypoint.z, dist, dir_x, dir_z))
            SubmitTrailblazeAction(player, controller, waypoint, pathfinding_state.current_waypoint)
        end
    -- 回退到旧逻辑
    elseif controller.RemoteDirectWalking then
        print(string.format("[ClientPathfinder] Remote move to waypoint %d: (%.1f, %.1f), dist: %.1f, dir: (%.2f, %.2f)",
            pathfinding_state.current_waypoint, waypoint.x, waypoint.z, dist, dir_x, dir_z))
        controller:RemoteDirectWalking(dir_x, dir_z)
    else
        print("[ClientPathfinder] Warning: no movement backend available")
    end
...
local function UpdatePathfinding()
    ...
    print("[ClientPathfinder] UpdatePathfinding called")
    MoveToNextWaypoint()
end
...
function ClientPathfinder.Start(target_x, target_z)
    ...
    print("[ClientPathfinder] Start called with target: (" .. target_x .. ", " .. target_z .. ")")
    ...
    pathfinding_state.stuck_counter = 0
    pathfinding_state.use_trailblazer = TrailblazerIntegration.loaded or TrailblazerIntegration.IsAvailable()
    pathfinding_state.last_trailblaze_waypoint = 0
    pathfinding_state.trailblaze_in_flight = false

    if pathfinding_state.use_trailblazer then
        print("[ClientPathfinder] Movement backend: Trailblazer")
    else
        print("[ClientPathfinder] Movement backend: RemoteDirectWalking (fallback)")
    end

    -- 开始更新任务
    if player.DoPeriodicTask then
        pathfinding_state.update_task = player:DoPeriodicTask(CONFIG.MOVE_INTERVAL, UpdatePathfinding)
        print("[ClientPathfinder] DoPeriodicTask scheduled with interval: " .. CONFIG.MOVE_INTERVAL)
    else
        print("[ClientPathfinder] Warning: DoPeriodicTask not available!")
    end
...
function ClientPathfinder.Stop()
    ...
    pathfinding_state.last_position = nil
    pathfinding_state.stuck_counter = 0
    pathfinding_state.use_trailblazer = false
    pathfinding_state.last_trailblaze_waypoint = 0
    pathfinding_state.trailblaze_in_flight = false
